...
//		cout << *(kernel + i) << endl;
    ...
    for (x = 0; x<X; x++)
        for (y = 0; y<Y; y++)
        {
            ...
            tmp[x + y*X] = val; //image after implementation of gaussian filter
        }
...
double chain(int from, int to, double * Ex, double * Ey,double * Gx, double * Gy, int X, int Y)
{
    ...
    // directions of gradients are different
                         ...
                         double * Gx, double * Gy, int X, int Y)
{
    ...
    for (x = 2; x<(X - 2); x++)
            ...
            //  modG -> |grad|
        {
            ...
            if (!Greater(R, mod) && gx >= gy && Greater(mod, L)) {tmp1++; Dx = 1;} /* H */
            else if (Greater(mod, D) && !Greater(U, mod) && gx <= gy) {tmp2++; Dy = 1;} /* V */
    ...
    // cout << "if:" << tmp1 << endl;
    // cout << "elif:" << tmp2 << endl;
    // cout << "Number of edge points:" << total << endl;
}
...
void chain_edge_points(int * next, int * prev, double * Ex,	double * Ey,double * Gx, double * Gy, int X, int Y)
{
    ...
    for (x = 2; x<(X - 2); x++)   /* 2 pixel margin to include the tested neighbors */
        for (y = 2; y<(Y - 2); y++)
            if (Ex[x + y*X] >= 0.0 && Ey[x + y*X] >= 0.0) /* must be an edge point */  // -1 by default
            {
                                ...
                                double * modG,	int X, int Y,double th_h, double th_l)
{
    ...
    for (i = 0; i<X*Y; i++)   /* prev[i]>=0 or next[i]>=0 implies an edge point */
        if ((prev[i] >= 0 || next[i] >= 0) && !valid[i] && modG[i] >= th_h) //th_h -> high threshold
        {
...
*/
void list_chained_edge_points(double ** x, double ** y, int * N, double ** theta, int ** curve_limits,
                              int * M,int * next, int * prev,	double * Ex, double * Ey, int X, int Y, double * Gx, double * Gy)
{
    int i, k, n;
    ...
    *theta = (double *)xmalloc(X * Y * sizeof(double));
    ...
    for (i = 0; i<X*Y; i++)   /* prev[i]>=0 or next[i]>=0 implies an edge point */
        if (prev[i] >= 0 || next[i] >= 0)
        {
            ...
            do
            {
                ...
                (*x)[*N] = Ex[k]; //Ex Ey position coordinates
                (*y)[*N] = Ey[k];
                (*theta)[*N] = atan2(Gy[k], Gx[k]);
                ++(*N);
                //N points
...
y[curve_limits[k]] == y[curve_limits[k+1] - 1].
*/
void devernay(double ** x, double ** y, int * N, double ** theta, int ** curve_limits,int * M,
              uchar * image, uchar * gauss, int X, int Y, double sigma, double th_h, double th_l)
{
    ...
    list_chained_edge_points(x, y, N, theta, curve_limits, M, next, prev, Ex, Ey, X, Y, Gx, Gy);
